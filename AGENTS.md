...
## 🎯 Назначение
SmetaBot автоматизирует защищённую выдачу смет подрядчиков клиентам в Telegram. Сервис создаёт приватные каналы с защитой контента, конвертирует документы в PNG 300 DPI с водяным знаком на выбор и отслеживает активность. Ниже — актуальное описание по состоянию на текущий код.

## 🧩 Компоненты и обязанности
### Telegram Bot (bot/)
- Telegram-бот на aiogram: стартовое меню и мастер публикации материалов для клиентов.
- Отвечает за пользовательские сценарии: загрузка PDF, выбор параметров рендера (DPI/водяной знак), запуск публикации и получение инвайт-ссылок.
- Взаимодействует с очередью задач (Celery/Redis) для рендера и отправки, и с БД (PostgreSQL) для хранения проектов, каналов и публикаций.
- Работает совместно с userbot (MTProto) для операций на стороне Telegram-каналов (создание/инвайты/состав).
- Поддерживает сценарии авторизации юзербота через куар код (см. login_userbot.py в корне).
